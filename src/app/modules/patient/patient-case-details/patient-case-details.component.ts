...
@Component({
  selector: 'app-patient-case-details',
  templateUrl: './patient-case-details.component.html',
  styleUrls: ['./patient-case-details.component.css']
})
export class PatientCaseDetailsComponent implements OnInit {
  ...
  getCaseAllDetails(){
    this.CaseService.getCaseAllDetails(this.caseId).subscribe(
      (data: any) => {
        ...
        //case status check
        if(this.caseDetails.status && this.caseDetails.status !== "CLOSED"){
          this.caseEditable = true;
        } else {
          this.caseEditable = false;
        } 
